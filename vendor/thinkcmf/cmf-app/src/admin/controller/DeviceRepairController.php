...
use think\Db;
use think\db\Query;
use app\admin\model\DeviceRepairModel;
use app\admin\model\DeviceRepairPlanModel;
class DeviceRepairController extends AdminBaseController
{
    ...
    public function index(){
        ...
        $list        = Db::table("mk_device_repair")
                            ->alias("a")
            ...
            ->where(function (Query $query) use ($filter) {
                ...
                if (!empty($keyword)) {
                    $query->where('c.device_name|a.sn|a.repair_worker', 'like', "%$keyword%");
                }
            })
    ...
    {
        $id                 = $this->request->param('id');
        $roles = Db::table("mk_device_repair")
                        ->alias("a")
                        ->join('mk_department b', 'a.department=b.id')
     ...
     * )
     */
    public function repair_addPost()
    {
        if ($this->request->isPost()) {
            $data      = $this->request->param();
            $data['create_time'] = time();
            $data['repair_time'] = strtotime($data['repair_time']);
            $linkModel = new DeviceRepairModel();
            if(isset($data['plan_id'])){
                $data['repair_type'] = "计划检修";
                if($linkModel->allowField(true)->save($data)){
                    $plan['last_repair_time'] = $data['repair_time'];
                    $plan['id'] = $data['plan_id'];
                    $planModel = new DeviceRepairPlanModel();
                    $planModel->allowField(true)->isUpdate(true)->save($plan);
                    $this->success("添加成功！",url("DeviceRepairPlan/plan",array('id'=>$data['device_id'])));
                }
            }
            else{
                $data['repair_type'] = "常规检修";
    ...
    public function repair_edit()
    {
        ...
        $order_goods = Db::table("mk_device_repair")
                        ->alias("a")
     ...
     * )
     */
    public function repair_editPost()
    {
        if ($this->request->isPost()) {
            $data      = $this->request->param();
            $data['repair_time'] = strtotime($data['repair_time']);
    ...
    public function repair_delete()
    {
        ...
        DeviceRepairModel::destroy($id);
        if($tag==0){
            $this->success("删除成功！", url("DeviceRepair/repair",array('id'=>$device_id)));
        }
        else{
            $this->success("删除成功！", url("DeviceRepair/index"));
        }
    }
