...
use app\admin\model\ProduceModel;
use think\Db;
use think\db\Query;
...
class ProduceController extends AdminBaseController
{
    ...
    //煤产量记录
    public function CoalOutputRecord()
    {
        /**搜索条件**/
    ...
    public function addCoalOutputRecord()
    {
        return $this->fetch();
    }

    public function addCoalOutputRecordPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                $ProduceModel->allowField(true)->save($data);
                $this->success("添加成功！", url("Produce/addCoalOutputRecord"));
            }
        }
    }
